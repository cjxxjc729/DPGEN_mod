...
#sys.path.append("/home/cjx//script/mmpp/")
...
def calculate_rot_av_from_refvec1_to_refvec2(refvec1,refvec2):
  ...
  if np.allclose(rotation_axis, [0, 0, 0]):
    print("oops!,rotation_axis=[0,0,0],man made one")
    if A2[0] == 0:
        w = np.array([1, 0, 0])
    elif A2[1] == 0:
        w = np.array([0, 1, 0])
        ...
        w = np.array([0, 0, 1])
    perp_vector = np.cross(A2, w)    
    rotation_axis = perp_vector / np.linalg.norm(perp_vector)  
    print("new rotation_axis=",rotation_axis)
