...
def collect_traj_4_dpgen_tar(src_directory, dest_directory):
  # 查找以 'traj' 结尾的文件
...
def collect_traj_4_dpgen(src_directory, target_directory):
    # 创建文件夹A
    folder_a_path = Path(target_directory) / "A"
    folder_a_path.mkdir(parents=True, exist_ok=True)
    
    # 在源目录中查找以"traj"结尾的文件并复制
    for file_path in Path(src_directory).glob("**/*traj"):
        shutil.copy(file_path, folder_a_path)
    
    # 获取目标位置中所有文件夹的名称，并找出最大的序号
    max_num = 0
    for item in Path(target_directory).iterdir():
    ...
    new_folder_path = Path(target_directory) / new_folder_name
    folder_a_path.rename(new_folder_path)
    
    return f"文件夹'A'已成功移动和重命名为: {new_folder_name}"
...
def sub_opt_by_jobs():
  ...
  if save_str_4_model_dev == True:  #如果save_str_4_model_dev == 'True'， 则收集traj文件4后续的DPGEN任
    ...
    dir_to_store_str_4_model_dev = input_dic['dir_to_store_str_4_model_dev']

    #print("compress traj")
    mkdir_if_not_exists(dir_to_store_str_4_model_dev)
    collect_traj_4_dpgen('./',dir_to_store_str_4_model_dev)
