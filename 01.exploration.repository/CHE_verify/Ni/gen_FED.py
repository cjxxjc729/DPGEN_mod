  ...
  while attempts < max_attempts:
    ...
    try:
      ...
      for key, value in products.items():
        test_values.append(value)
      #print(test_values)
      if not any('x' in str(item) for item in test_values) and all(value == 1 for key, value in reactants.items() if 'Ar' in key) and all(value == 1 for key, value in products.items() if 'Ar' in key) and not any('.' in str(item) for item in test_values):
        break
